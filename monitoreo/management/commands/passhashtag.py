...
from monitoreo.models import Hashtag
#from monitoreo.utils import mismo_dia
import os
import datetime
import tweepy
        ...
        while boolean:
            try:
                if h:
                    ...
                    busq = nombre
                #Buscar la forma de mejorar esta linea, search no entrega mas de 100 tweets
                tweets = api.search(q=busq,rpp=100)

                #if len(tweets)==0:
                #    boolean = False
                boolean = False #Mientras lo anterior no se cambie
